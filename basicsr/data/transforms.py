...
def paired_random_crop_gaussian(img_gts, img_lqs, gt_patch_size, scale, gt_path):
    """Paired random crop.
    ...
    # randomly choose from truncated gaussian
    mu_h = (h_lq - lq_patch_size) / 2
    mu_w = (w_lq - lq_patch_size) / 2
    sigma_h = (mu_h / 2) / 2
    sigma_w = (mu_w / 2) / 2

    top = int(scipy.stats.truncnorm.rvs((0-mu_h)/sigma_h,(h_lq - lq_patch_size-mu_h)/sigma_h, loc=mu_h, scale=sigma_h))
    left = int(scipy.stats.truncnorm.rvs((0-mu_w)/sigma_w,(w_lq - lq_patch_size-mu_w)/sigma_w, loc=mu_w, scale=sigma_w))
...
def augment_triplet(imgs, kernels, hflip=True, rotation=True, return_status=False, vflip=False):
    """Augment: horizontal flips OR rotate (0, 90, 180, 270 degrees).
    ...
    kernels = _augment_kernel(kernels)
