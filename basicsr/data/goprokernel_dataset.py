...
from basicsr.data.transforms import augment, augment_triplet, paired_random_crop_gaussian, paired_random_crop, triplet_random_crop
...
class GoProKernelDataset(data.Dataset):
    ...
    def __init__(self, opt):
        super(GoProKernelDataset, self).__init__()
        self.opt = opt
    ...
    def interpolate_kernel(self, kernel):
        #ksize = self.ksize
        K, _, H, W = kernel.shape
        kernel = Rearrange('k d h w -> (h w) d k')(kernel)
        kernel = F.interpolate(kernel, mode='linear', size=self.ksize)

        kernel = Rearrange('(h w) d k-> k d h w', h=H, w=W)(kernel)
    ...
    def __getitem__(self, index):
        ...
        #img_crop_list, kernel_crop = augment_triplet([img_gt_out, img_lq_out], kernel_crop, True, True)
        #img_gt_out, img_lq_out = img_crop_list
